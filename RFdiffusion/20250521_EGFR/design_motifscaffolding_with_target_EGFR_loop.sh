...
apptainer exec --nv -B /pasteur  "${sif}" bash -c \
    "python3.9 ${src_dir} \
    inference.output_prefix=pdb_outputs_loop/EGFR_loop \
    inference.input_pdb=input_pdbs/4uv7.pdb \
    'contigmap.contigs=[50/A351-363/50]' \
    contigmap.length=70-120  inference.num_designs=100 inference.ckpt_override_path=../../models/Complex_base_ckpt.pt"
